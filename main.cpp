...
bool parser(unsigned char* buf, int size)
{
    ...
    //parse ip header
    int ipHeaderLength = (buf[0] & 0xf) * 4;
    buf += ipHeaderLength;
    size -= ipHeaderLength;

    //parse tcp port
    int tcpDstPort = buf[2] * 0x100 + buf[3];
    ...
    if(tcpDstPort == 0x05)
    {
        printf("packet is http\n");
        buf += tcpHeaderLength;
        size -= tcpHeaderLength;

        printf("http packet check\n");
        for(i=0;i<size;i++)
        {
            //print 16 bites per line
            if(i % 0x10 == 0)
            {
                printf("\n");
            }
        ...
        for(i=0;i<size-5;i++)
        {
            if(strncmp(reinterpret_cast<const char*>(buf+i), "Host_",5) == 0)
            {
                buf += i+6;
                size -= i+6;
...
static bool filter_pkt (struct nfq_data *tb)
{
	...
	if (ret >= 0)
        printf("\n");
        // dump(data, ret);
		bool is_filtered = parser(data, ret);
